...
describe("Airdrop Contract", function () {
    ...
    const TOTAL_AIRDROP_ALLOCATION = ethers.parseUnits("500000000", 8); // 500 million tokens as per PDF
    ...
    it("Should allow owner to airdrop tokens to a recipient", async function () {
        const airdropAmount = ethers.parseUnits("1000", 8);
    ...
    it("Should not allow a non-owner to airdrop tokens", async function () {
        const airdropAmount = ethers.parseUnits("100", 8);
    ...
    it("Should not allow airdropping more than the total allocation", async function () {
        const excessiveAmount = TOTAL_AIRDROP_ALLOCATION + ethers.parseUnits("1", 8);
    ...
    it("Should not allow airdropping if contract balance is insufficient", async function () {
        ...
        const LIMITED_ALLOCATION = ethers.parseUnits("10000", 8); // A smaller allocation for this test
        ...
        const fundedAmount = ethers.parseUnits("5000", 8);
        ...
        const airdropAmount = ethers.parseUnits("6000", 8); // > fundedAmount, < LIMITED_ALLOCATION
    ...
    it("Should allow owner to recover accidentally sent ERC20 tokens", async function () {
        ...
        const dummyAmount = ethers.parseUnits("500", 8); // Match AimondToken's 8 decimals
    ...
    it("Should not allow recovering the main AimondToken via recoverERC20", async function () {
        ...
        await expect(airdropContract.connect(owner).recoverERC20(await aimondToken.getAddress(), ethers.parseUnits("1", 8)))
